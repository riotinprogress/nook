...
export function ThemeGallery({ themeId, alts, customs, onPick, onCycle, onEdit, onDelete }: GalleryProps) {
  ...
  const list = useMemo(() => {
    ...
    return [...mine, ...stock].filter(
      (t) =>
        (mode === 'all' || (mode === 'mine' ? t.custom : mode === 'favorites' ? favorites.has(t.id) : mode === 'dark' ? t.dark : !t.dark)) &&
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting) {
          setLoadedCount((c) => Math.min(c + 36, list.length));
        }
      },
      { threshold: 0.1 }
    );
  ...
  const shown = list.slice(0, loadedCount);
  ...
  return (
    <>
      <div className="theme-filters">
        ...
        <button
          className="columns-button secondary-button"
          onClick={() => setColumns((c) => (c === 5 ? 3 : c + 1))}
          title={`Cycle columns: ${columns} → ${columns === 5 ? 3 : columns + 1}`}
          aria-label={`Cycle grid columns, currently ${columns}`}
        >
          <Icon name="grid" size={14} />
          {columns} columns
        </button>
        <button
          className="favorites-button secondary-button"
          onClick={() => change(() => setMode('favorites'))}
          title="View favorited themes"
          aria-label="View favorited themes"
        >
      ...
      {!list.length && (
        <p className="theme-empty">
          {mode === 'mine' ? 'Edit any theme with the cog to save your own version here.' : mode === 'favorites' ? 'Click the star on any theme to add it to your favorites.' : 'No themes match that search.'}
        </p>
      )}
      ...
      <div
        ...
        style={{ gridTemplateColumns: `repeat(${columns}, minmax(250px, 1fr))`, gap: '11px' }}
      >
        {shown.map((t) => {
